...
fn convert_path_for_cli(p: &std::path::Path) -> String {
...
pub async fn generate_tts(
    ...
    app.emit(
        "tts_status",
        serde_json::json!({"progress": 5, "status": "starting"}),
    )
    .ok();
    ...
    app.emit(
        "tts_status",
        serde_json::json!({"progress": 15, "status": "synthesizing (edge-tts)"}),
    )
    .ok();
    log_info!(
        "TTS",
        "Running edge-tts: python {:?} {:?}",
        python_path,
        edge_args
    );
    let edge_status = create_hidden_command(&python_path)
        ...
        .map_err(|e| {
            app.emit(
                "tts_status",
                serde_json::json!({"progress": 0, "status": format!("error_edge_tts: {}", e)}),
            )
            .ok();
    ...
    if !edge_status.success() {
        app.emit(
            "tts_status",
            serde_json::json!({"progress": 0, "status": "error_edge_tts"}),
        )
        .ok();
    ...
    if mode == "normal" {
        ...
        let audio_data =
            std::fs::read(&tts_path).map_err(|e| format!("Failed to read audio file: {}", e))?;
    ...
    app.emit(
        "tts_status",
        serde_json::json!({"progress": 50, "status": "enhancing (rvc)"}),
    )
    .ok();
    let model = if let Some(m) = model_file {
        m
    } else {
        let cfg = load_tts_settings(app.clone())
            .await
            .unwrap_or_else(|_| serde_json::json!({}));
        cfg.get("selectedModel")
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .to_string()
    };
    if model.is_empty() {
        ...
        app.emit(
            "tts_status",
            serde_json::json!({"progress": 0, "status": "error_model_not_selected"}),
        )
        .ok();
    ...
    if !model_path.exists() {
        app.emit(
            "tts_status",
            serde_json::json!({"progress": 0, "status": "error_model_missing"}),
        )
        .ok();
    ...
    let mut rvc_args = vec![
        ...
        convert_path_for_cli(&tts_path),
        "-o".into(),
        convert_path_for_cli(&rvc_path),
        "-mp".into(),
        convert_path_for_cli(&model_path),
    ];
    ...
    app.emit(
        "tts_status",
        serde_json::json!({"progress": 60, "status": "converting (rvc)"}),
    )
    .ok();
    log_info!(
        "TTS",
        "Running RVC: python -m rvc_python cli args: {:?}",
        rvc_args
    );
    let rvc_status = create_hidden_command(&python_path)
        ...
        .map_err(|e| {
            app.emit(
                "tts_status",
                serde_json::json!({"progress": 0, "status": format!("error_rvc: {}", e)}),
            )
            .ok();
    ...
    if !rvc_status.success() {
        app.emit(
            "tts_status",
            serde_json::json!({"progress": 0, "status": "error_rvc"}),
        )
        .ok();
    ...
    let audio_data =
        std::fs::read(&rvc_path).map_err(|e| format!("Failed to read RVC audio file: {}", e))?;
    let base64_audio = general_purpose::STANDARD.encode(&audio_data);
...
pub async fn generate_static_tts_file(
    ...
    if trimmed.is_empty() {
        app.emit(
            "tts_status",
            serde_json::json!({"progress": 0, "status": "error_empty_text"}),
        )
        .ok();
    ...
    let (_venv_dir, python_path) = match venv_paths(&app) {
        ...
        Err(e) => {
            app.emit(
                "tts_status",
                serde_json::json!({"progress": 0, "status": format!("error_env: {}", e)}),
            )
            .ok();
            return Err(e);
        }
    ...
    if mode == "normal" {
        ...
        let edge_args = [
            ...
            &trimmed,
            "--write-media",
            &convert_path_for_cli(&abs_path),
        ];

        app.emit(
            "tts_status",
            serde_json::json!({"progress": 15, "status": "edge_tts_start"}),
        )
        .ok();
        log_info!(
            "TTS",
        ...
        app.emit(
            "tts_status",
            serde_json::json!({"progress": 100, "status": "done"}),
        )
        .ok();
        ...
        app.emit(
            "tts_status",
            serde_json::json!({"progress": 50, "status": "rvc_start"}),
        )
        .ok();
        ...
        if !src_wav.exists() {
            app.emit(
                "tts_status",
                serde_json::json!({"progress": 0, "status": "error_rvc_missing_output"}),
            )
            .ok();
            return Err(format!(
                "RVC output file does not exist: {}",
                src_wav.display()
            ));
        }
        ...
        app.emit(
            "tts_status",
            serde_json::json!({"progress": 75, "status": "rvc_converted"}),
        )
        .ok();
        ...
        if want_format == "mp3" {
            app.emit(
                "tts_status",
                serde_json::json!({"progress": 80, "status": "transcoding"}),
            )
            .ok();
                ...
                if st.success() {
                    ...
                    match ff_status {
                        Ok(s) if s.success() => {
                            ...
                            let rel_path =
                                format!("static_audios/{}/{}", safe_redemption, mp3_name);
                            app.emit(
                                "tts_status",
                                serde_json::json!({"progress": 100, "status": "done"}),
                            )
                        ...
                        Ok(_) => {
                            app.emit(
                                "tts_status",
                                serde_json::json!({"progress": 90, "status": "transcode_failed"}),
                            )
                            .ok();
                        }
                        Err(e) => {
                    ...
                    app.emit(
                        "tts_status",
                        serde_json::json!({"progress": 80, "status": "transcode_skipped"}),
                    )
                    .ok();
                }
            } else {
