...
pub async fn handle_connection(
    ...
    log_and_emit(
        ...
        role,
        "CONNECTION_ENDED",
        "Connection loop ended, cleaning up",
    )
    .await;
    clear_shared_connection_state(&window).await;
    window.emit("CLIENT_DISCONNECTED", ()).ok();
...
async fn handle_decrypted(window: &Window, plaintext: String) {
    if let Ok(msg) = serde_json::from_str::<crate::state::Message>(&plaintext) {
        match msg {
            ...
            crate::state::Message::ServerMessage {
                audio,
                title,
                content,
            } => {
                let payload = json!({
                    "id": format!("server_message_{}", Utc::now().timestamp_millis()),
                    "title": title.unwrap_or_else(|| "Server Message".to_string()),
                    "content": content,
                    "audioData": general_purpose::STANDARD.encode(&audio),
                    "source": "server",
                });
                let _ = window.emit("SERVER_MESSAGE_RECEIVED", payload);
                return;
            }
            _ => {}
        }
    ...
    if let Some(keys) = session_keys {
        ...
        match serde_json::to_string(&server_msg) {
            Ok(serialized) => match encrypt_message(keys, &serialized).await {
                ...
                Err(e) => eprintln!(
                    "[SERVER_MESSAGE_ERROR] Failed to encrypt server message: {}",
                    e
                ),
            },
            Err(e) => eprintln!(
                "[SERVER_MESSAGE_ERROR] Failed to serialize server message: {}",
                e
            ),
        }
    }
