  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div
          className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm ${statusColor}`}
        >
      ...
      <div className="mt-6 flex flex-col items-center text-center">
        <div className="relative h-14 w-14">
          ...
          {paused && (
            <div className="absolute inset-0 rounded-full border-2 border-amber-400/70" />
          )}
        </div>
...
type TimerActionPayload =
  ...
  | {
      type: "adjust";
      adjustment: { type: "subtract"; seconds: number } | { type: "clear" };
    };
...
const ClientPage = () => {
  ...
  const [latestRedemption, setLatestRedemption] =
  ...
  const applyTimerAction = (
    ...
    if (descriptor.kind === "start") {
      ...
      addLog(
        "info",
        `Timer started: ${formatSecondsToTime(totalDuration)} for "${context.title}"`,
      );
      return;
    }
    ...
    if (descriptor.adjustment.kind === "clear") {
      ...
      if (removedCount === 0) {
        ...
        addLog(
          "success",
          `Cleared ${removedCount} active timer${removedCount === 1 ? "" : "s"}`,
        );
      }
      return;
    }
    ...
    if (summary.adjusted > 0) {
      parts.push(
        `reduced ${summary.adjusted} timer${summary.adjusted === 1 ? "" : "s"}`,
      );
    }
    if (summary.removed > 0) {
      parts.push(
        `removed ${summary.removed} timer${summary.removed === 1 ? "" : "s"}`,
      );
    }
    addLog(
      "success",
      ...
      if (audioElement) {
        ...
        if ("setSinkId" in audioElement && audioDeviceId !== "default") {
          try {
            ...
            addLog(
              "info",
              "Using default audio device (device selection not supported)",
            );
          }
        }
  ...
  const handleIncomingTts = async (
    ...
    if (
      ...
      typeof redemption.audioData === "string" &&
      redemption.audioData.trim() !== ""
    ) {
  ...
  useEffect(() => {
    ...
    const unlistenServerMessage = listen(
  ...
  const handleTestConnection = async (addrOverride?: string) => {
    const target = (addrOverride ?? serverAddress).trim();
    if (!target) {
      setConnectionTestStatus("No server address provided");
      addLog("error", "No server address provided for test");
      return;
    }
    ...
    try {
      const result = (await invoke("probe_connection", {
        address: target,
        timeoutMs: 3000,
      })) as string;
      setConnectionTestStatus(result);
      addLog("success", `Connection test OK: ${result}`);
    } catch (error) {
      const errMsg =
        typeof error === "string"
          ? error
          : (error as any)?.toString?.() || "Unknown error";
      setConnectionTestStatus(errMsg);
      addLog("error", `Connection test failed: ${errMsg}`);
    } finally {
  ...
  const handleConfirmPairing = async () => {
    try {
      ...
      addLog(
        "info",
        "Pairing confirmed locally. Waiting for the other side to confirm and establish session...",
      );
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      try {
        await invoke("send_disconnect_notice", {
  ...
  const persistRecentServer = async (addr: string) => {
    ...
    try {
      ...
      let current: string[] =
        ((await settingsStoreRef.current.get("recentServers")) as string[]) ||
        [];
  ...
  useEffect(() => {
    const loadClientSettings = async () => {
      try {
        const { load } = await import("@tauri-apps/plugin-store");
        const store = await load("client-settings.json", {
          autoSave: true,
          defaults: {},
        });
        settingsStoreRef.current = store;
        const recents = (await store.get("recentServers")) as
          | string[]
          | undefined;
        const autoEn = (await store.get("autoConnectEnabled")) as
          | boolean
          | undefined;
        const autoAddr = (await store.get("autoConnectAddress")) as
          | string
          | undefined;
  ...
  const runAutoAttempt = () => {
    ...
    if (connectInProgressRef.current || isConnecting) {
      console.log(
        "Connection attempt already in progress, skipping auto attempt",
      );
      scheduleNextAutoRun();
      return;
    }
    ...
    addLog(
      "info",
      `Auto-connect attempt #${autoConnectAttemptsRef.current} to ${target}`,
    );
    ...
    attemptTimeoutRef.current = setTimeout(() => {
      ...
      if (
        (isConnecting || connectInProgressRef.current) &&
  ...
  const scheduleNextAutoRun = () => {
    ...
    addLog(
      "info",
      `Next auto-connect attempt #${attempt + 1} in ${(finalDelay / 1000).toFixed(1)}s`,
    );
        ...
        <div className="flex items-center justify-between px-8 py-6">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center">
              <div
                className={`w-3 h-3 rounded-full mr-3 ${connectionState === "connected" ? "bg-green-400" : "bg-red-400"}`}
              ></div>
              <h1 className="text-xl font-semibold text-white">Client Mode</h1>
            </div>
            <div className="flex items-center gap-3">
              <div
                className={`px-3 py-1.5 rounded-full text-xs font-semibold ${connectionState === "connected" ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-red-500/20 text-red-400 border border-red-500/30"}`}
              >
            ...
            >
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-blue-500/30 flex items-center justify-center">
                      ...
                      >
                        <path
                          fill="currentColor"
                          d="M7 2h2v6H7V2m8 0h2v6h-2V2M6 8h12v3a5 5 0 0 1-5 5v4h-2v-4a5 5 0 0 1-5-5V8z"
                        />
                      </svg>
                    </div>
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-400">
                        Automatically connect to your preferred server
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <div
                      ...
                      >
                        <motion.span
                          className={`absolute top-0.5 w-6 h-6 rounded-full bg-white shadow-lg transform transition-all duration-300 flex items-center justify-center ${
                            autoConnectEnabled ? "left-7" : "left-0.5"
                          }`}
                          initial={false}
                          animate={{
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <input
                            ...
                            onChange={(e) =>
                              handleAutoConnectAddressChange(e.target.value)
                            }
                            placeholder={
                              recentServers[0] || "192.168.1.100:12345"
                            }
                ...
                <motion.button
                  ...
                  disabled={isTestingConnection || !serverAddress.trim()}
                  className="w-full bg-gray-700/60 hover:bg-gray-700 text-gray-200 font-medium py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isTestingConnection ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                ...
                {connectionTestStatus && (
                  <div className="text-xs text-gray-300 bg-gray-900/40 border border-gray-700/60 rounded-lg px-3 py-2">
                    {connectionTestStatus}
                  </div>
                )}
              </div>
              ...
              <ConnectingHero
                ...
                nextRetrySec={
                  nextRetryDelayMs !== null
                    ? Math.round(nextRetryDelayMs / 1000)
                    : null
                }
          ...
          {connectionState === "pairing" && pairingCode && (
            ...
            >
              ...
              <p className="text-gray-300 mb-4">
                Enter this pairing code on the server to establish a secure
                connection:
              </p>
              ...
              <p className="text-sm text-gray-400">
                After confirming, please wait for the other side. The connection
                will complete automatically once both sides confirm.
              </p>
            </motion.div>
          )}
            ...
            <div className="flex flex-col lg:flex-row h-[calc(100vh-200px)] gap-6">
                  ...
                  {latestRedemption ? (
                    <div className="bg-gradient-to-br from-gray-800/70 to-gray-900/70 border border-gray-700/50 rounded-2xl p-10 shadow-lg relative">
                      <div className="absolute top-4 right-4 flex items-center gap-2 text-xs text-gray-400">
                        ...
                        <span>
                          {latestRedemption.receivedAt.toLocaleTimeString()}
                        </span>
                      </div>
                      ...
                      {latestRedemption.timerAction.kind === "start" && (
                        <div className="flex items-center justify-center gap-3 mb-4">
                          ...
                          <span className="text-2xl font-mono text-orange-400">
                            {formatSecondsToTime(
                              latestRedemption.timerAction.durationSeconds,
                            )}
                          </span>
                        </div>
                      )}
                      {latestRedemption.timerAction.kind === "adjust" && (
                        <div className="flex items-center justify-center gap-3 mb-4">
                          ...
                          <span className="text-sm font-medium text-yellow-300">
                            {latestRedemption.timerAction.adjustment.kind ===
                            "clear"
                    ...
                    <div className="text-center text-gray-500">
                      ...
                      <p className="text-sm mt-2 text-gray-600">
                        They will appear here in large format once received.
                      </p>
                    </div>
                  )}
                </motion.div>
              ...
              <div className="w-full lg:w-80 xl:w-96 flex-shrink-0">
                <motion.div
                  ...
                  className="h-full flex flex-col bg-gray-800/40 border border-gray-700/40 rounded-xl p-6"
                >
                  ...
                  <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                    {Object.keys(activeTimers).length === 0 ? (
                      ...
                      Object.values(activeTimers).map((timer) => {
                        const timeDisplay = formatSecondsToTime(
                          timer.remainingTime,
                        );
                        return (
                          <div
                            key={timer.id}
                            className="group bg-gray-800/60 border border-gray-700/50 hover:border-gray-600/60 rounded-xl px-4 py-3 transition"
                          >
                              ...
                              <div className="flex flex-col items-end">
                                ...
                                {typeof timer.totalDuration === "number" && (
                                  <span className="mt-1 text-[10px] text-gray-400">
                                    / {Math.round(timer.totalDuration)}s
                                  </span>
                                )}
                              </div>
                            </div>
                              ...
                              timer.totalDuration > 0 && (
                                <div className="mt-2 h-1.5 w-full bg-gray-700/60 rounded-full overflow-hidden">
                                  <div
                                    ...
                                    style={{
                                      width: `${Math.min(100, Math.max(0, (1 - timer.remainingTime / timer.totalDuration) * 100))}%`,
                                    }}
                                  />
                                </div>
                              )}
            ...
            >
              <div className="absolute -top-10 right-0 flex gap-2">
                <motion.button
                  ...
                  className="px-3 py-2 rounded-t-lg bg-gray-800/80 backdrop-blur border border-gray-700/60 text-xs font-medium flex items-center gap-2 text-gray-300 shadow-lg"
                >
              ...
              <motion.div
                ...
                className="bg-gray-900/85 backdrop-blur-md border border-gray-700/60 rounded-xl shadow-2xl p-4 w-full sm:min-w-[480px] max-h-56 flex flex-col"
              >
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-sm font-semibold text-white flex items-center gap-2">
                    <span className="p-1.5 bg-cyan-500/20 rounded">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setAutoScrollLog((a) => !a)}
                      className={`text-[10px] px-2 py-1 rounded border ${autoScrollLog ? "border-green-400/50 text-green-400 bg-green-500/10" : "border-gray-500/40 text-gray-400 hover:bg-gray-700/40"}`}
                    >
                      {autoScrollLog ? "Auto" : "Manual"}
                    </button>
                    <button
                      onClick={() => setLogs([])}
                      className="text-[10px] px-2 py-1 rounded border border-red-400/50 text-red-400 hover:bg-red-500/10"
                    >
                      Clear
                    </button>
                  </div>
                ...
                <div
                  ref={logContainerRef}
                  className="flex-1 overflow-y-auto font-mono text-[11px] leading-relaxed pr-1 space-y-0.5 custom-scrollbar"
                >
                  {logs.length === 0 ? (
                    ...
                    logs.map((log, i) => (
                      <div
                        key={i}
                        className={`${log.type === "error" ? "text-red-400" : log.type === "success" ? "text-green-400" : "text-cyan-400"}`}
                      >
